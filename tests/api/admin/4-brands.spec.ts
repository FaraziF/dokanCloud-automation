...
test.use({ extraHTTPHeaders: { Authorization: `Bearer ${String(process.env.Admin_API_TOKEN)}`, strategy: "admin" } });
	...
	test('create a new brands', async () => {
		const [response, responseBody] = await apiUtils.post(endPoints.createBrands, { data: payloads.createBrands() });
		expect(response.ok()).toBeTruthy();
		expect(responseBody).toBeTruthy();
	...
	test('edit brand', async () => {
		const [response, responseBody] = await apiUtils.put(endPoints.editBrand(brandID), { data: payloads.editBrand() });
		expect(response.ok()).toBeTruthy();
		expect(responseBody).toBeTruthy();
	});
	test('delete brand', async () => {
		const [response, responseBody] = await apiUtils.delete(endPoints.deleteBrand(brandID));
		expect(response.ok()).toBeTruthy();
		expect(responseBody).toBeTruthy();
        console.log(await response.json())
	});
