...
export class ProductBulkEdit extends BasePage {
    ...
    async isOpenChatVisible(): Promise<boolean> {
      const closeChatButton = await this.page.getByLabel('Open chat').getByRole('button')
    ...
    async updateProductBasicInfo(productTitle: string, productTitleUpdate: string) {
      const productNameCell = this.page.getByRole('cell', { name: productTitle, exact: true });
      const productNameInput = await productNameCell.getByRole('textbox');
      const productTitleLocator = this.page.locator("(//td[@data-is-active='false'])[1]")
      await this.page.locator("(//td[@data-is-active='false'])[1]").click()
      await this.page.locator("(//input[contains(@class,'block h-full')])[1]").fill('')
    ...
    async updateProductPricingDetails(regular: string, cost: string, sale: string) {
      await this.page.getByRole('cell', { name: '5' }).getByRole('spinbutton').fill(regular);
      await this.page.getByRole('spinbutton').nth(1).fill(cost);
      await this.page.getByRole('spinbutton').nth(2).fill(sale);
    }
    
    async updateInventoryDetails(newSKU: string, newBarcode: string, newStockQuantity: string, newLowStockThreshold: string) {
    ...
    async updateProductSEO(productTitle: string, updateSEOTitle: string) {
      await this.page.getByRole('cell', { name: productTitle, exact: true }).getByRole('textbox').fill(updateSEOTitle);
    }
    ...
    async verifyProductBulkEdit(productTitleUpdate: string, newSKU: string, newBarcode: string, newStockQuantity: string, updateSEOTitle: string) {
      ...
      // Validate changes
      await expect(this.page.getByLabel('Product name')).toHaveValue(productTitleUpdate ?? '');
