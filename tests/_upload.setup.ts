...
// export async function uploadsetup() {
  ...
  setup.describe('Upload banner and logo', () => {
    setup.use({ extraHTTPHeaders: { Authorization: `Bearer ${String(process.env.Vendor_API_TOKEN)}`, strategy: "vendor" } });
    ...
    const logoImageBuffer = fs.readFileSync(logoImagePath);
       ...
       setup('Upload Store banner', async () => {
           ...
           const [ response, responseBody ] = await apiUtils.post(endPoints.vendorUploadBannerImage, {
             multipart: {
               file: {
                 name: 'apple.png',
                 mimeType: 'image/png',
       ...
       setup('Upload Logo ', async () => {
           // Send the POST request with banner and logo images
           const [ response, responseBody ] = await apiUtils.post(endPoints.vendorUploadLogoImage, {
             multipart: {
               file: {
                 name: 'logo.png',
                 mimeType: 'image/png',
     ...
     setup('Save banner and logo image', async () => {
       ...
       const [response, responseBody] = await apiUtils.put(endPoints.vendorSaveBanerImage, {data: payloads.vendorSaveBannerImage(bannerID, logoID)})
       expect(response.ok()).toBeTruthy();
       expect(responseBody).toBeTruthy();
