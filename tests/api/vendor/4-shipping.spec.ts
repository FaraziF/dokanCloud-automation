...
test.use({ extraHTTPHeaders: { Authorization: `Bearer ${String(process.env.Vendor_API_TOKEN)}`, strategy: "vendor" } });
	...
	test('Get Shipping Preferences', async () => {
		const [response, responseBody] = await apiUtils.get(endPoints.vendorGetShippingPreferences);
		expect(response.ok()).toBeTruthy();
		expect(responseBody).toBeTruthy();
	});
	test('Save Shipping Preferences', async () => {
		const [response, responseBody] = await apiUtils.put(endPoints.vendorSaveShippingPreferences, {data: payloads.saveShippingPreference()});
		expect(response.ok()).toBeTruthy();
		expect(responseBody).toBeTruthy();
	});
  test('Get Shipping Profile', async () => {
		const [response, responseBody] = await apiUtils.get(endPoints.vendorGetShippingProfile);
		expect(response.ok()).toBeTruthy();
		expect(responseBody).toBeTruthy();
...
// 74, 71 production
  ...
  test('Get Individual Shipping Profile', async () => {
		const [response, responseBody] = await apiUtils.get(endPoints.vendorViewIndividualShippingProfile(shippingProfileID));
		expect(response.ok()).toBeTruthy();
		expect(responseBody).toBeTruthy();
	});
...
// hide this scenario because: we don't know which one is weight or prices basaed
  ...
  test('Get Shipping Address', async () => {
    const [response, responseBody] = await apiUtils.get(endPoints.vendorGetShippingAddress);
    expect(response.ok()).toBeTruthy();
    expect(responseBody).toBeTruthy();
  });
