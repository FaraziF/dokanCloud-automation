import { test, request, expect, type Page, type APIRequestContext, APIResponse, Request } from "@playwright/test";
...
test.describe("Vendor Onboarding Process", () => {
    ...
    test('Complete vendor onboard process', async () => {
        ...
        const loginData = {
            ...
            const [response, responseBody] = await apiUtils.post(endPoints.vendorLogin, { data: loginData });
        ...
        const [response1, responseBody1] = await apiUtils.get(endPoints.vendorDetails, {
            headers: {
        ...
        const [response2, responseBody2] = await apiUtils.patch(endPoints.vendorMarkAsTest(vendorCreatorID), {
            data: { 'state': true }, 
        ...
        await vendorPage.goToNextOnboardProcess();
        await expect(page).toHaveURL(/.*\/vendor\/onboarding\/contact-verification/);
