...
teardown.describe('get api test coverage', () => {
  ...
  teardown('get coverage', async () => {
    const endpoints = [
      endPoints.getdashboardURL,
      endPoints.getstorefrontURL,
      endPoints.getcatalogURL,
      endPoints.getshippingURL,
      endPoints.getuserURL,
      endPoints.getpaymentURL,
      endPoints.gettaxURL,
      endPoints.getreportURL,
      endPoints.getorderURL,
      endPoints.getintegrationURL,
      endPoints.getcontentURL,
      endPoints.getcouponURL,
      endPoints.getcartURL,
    ];
    const allRoutes: string[] = [];
    // const allRouteObjValues = [];
    const allRouteObjValues: any[] = [];
    ...
    const allRouteMethods: string[][] = allRouteObjValues.map(
      (route: any) => route.methods
    );
    let coverageArray = allRouteMethods.flatMap((methods, i) =>
      methods.map((method) => `${method} ${allRoutes[i]}`)
    );
...
function getCoverage(coverageArray: any[], outputFile?: string) {
  ...
  for (const route of coverageArray) {
    ...
    const output = execSync(`grep -irl -E '${pattern}' tests/api | cat  `, {
      encoding: 'utf-8',
    });
    if (output.toString() != '') {
      coveredEndPoints += 1;
  ...
  const percentCovered =
    ((coveredEndPoints / totalEndPoints) * 100).toFixed(2) + '%';
