...
export class VendorPage extends BasePage {
  ...
  async createStandardProduct(productInfo: {
    productName: () => string;
    ...
    await this.page
      .locator(selector.product.sku)
      .fill(data.product.standard.sku());
  ...
  async selectIndividualBusinessType() {
    ...
    const countrySelect = this.page.locator(
      "(//div[contains(@class,'css-15t20dc-control')])[1]"
    );
  ...
  async registerNewVendor() {
    ...
    await this.page
      .getByPlaceholder('Store Name')
      .fill(faker.word.adjective() + ' Store');
    await this.page.locator('#country svg').click();
    await this.page.locator('#react-select-2-input').fill('United States');
    await this.page.getByRole('option', { name: 'United States' }).click();
    await this.page.getByPlaceholder('Enter an address').click();
    ...
    const getStoreAddress = await this.page
      .getByText('Texas Tech University')
      .first();
