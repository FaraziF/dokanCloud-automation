...
test.describe("Calculation Test", () => {
    ...
    test("Get individual Product Price, Tax and Shipping", async ({ request }) => {
        const _response = await request.get(endPoints.individualProductGet(product_id))
    ...
    test("Get Shipping price", async ({ request }) => {
        ...
        expect(_response.ok()).toBeTruthy()
            ...
            _existingShippingCost = _shippingDefaultCost + _shippingHandlingFee + _shippingWeightCost
    ...
    test("Get subscription price", async({ request }) => {
        ...
        console.log("Existing Admin Comission Amount:" + _existingTotalAdminComissionAmount)
    })


  // Checkout page: need to validation tax. tax calculation validation with existing set tax
    test("Get Tax price", async({ request }) => {
        const _response = await request.get(endPoints.getTax, {headers: adminAuth })
        expect(_response.ok()).toBeTruthy()
        const res = await _response.json()
    ...
    test("Create Order", async({ request }) => {
        let _response = await request.post(endPoints.pay, {data: payloads.checkout(_cartID ), headers:customerAuth })
        // console.log(await _response.json()) 
        const res = await _response.json()
        expect(_response.ok()).toBeTruthy()
        // console.log(res)
        ...
        _currentTotalAdminComissionAmountParse =  parseFloat(_currentTotalAdminComissionAmount)
        ...
        _currentVendorEarning = _currentProductSalesPriceParse - _currentTotalAdminComissionAmountParse;
        ...
        console.log("Current Admin Comission Amout: " + _currentTotalAdminComissionAmountParse)
    ...
    test("Admin Comission Match", async({ request }) => {
        // Admin Comission 
        if(_existingTotalAdminComissionAmount === _currentTotalAdminComissionAmountParse) {
