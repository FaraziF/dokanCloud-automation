...
test.use({ extraHTTPHeaders: { Authorization: `Bearer ${String(process.env.Admin_API_TOKEN)}`, strategy: "admin" } });
	...
	test('get all subscription', async () => {
		const [response, responseBody] = await apiUtils.get(endPoints.getAllSubscription);
		expect(response.ok()).toBeTruthy();
		expect(responseBody).toBeTruthy();
	});
	test('get published subscription', async () => {
		const [response, responseBody] = await apiUtils.get(endPoints.getPublishSubscription);
		expect(response.ok()).toBeTruthy();
		expect(responseBody).toBeTruthy();
	});
	test('get draft subscription', async () => {
		const [response, responseBody] = await apiUtils.get(endPoints.getDraftSubscription);
		expect(response.ok()).toBeTruthy();
		expect(responseBody).toBeTruthy();
	});
	...
	test('delete susbcription', async () => {
		const [response, responseBody] = await apiUtils.delete(endPoints.deleteCreatedSubscription(subscriptionID));
		expect(response.ok()).toBeTruthy();
		expect(responseBody).toBeTruthy();
