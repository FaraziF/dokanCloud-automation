...
test.describe("Category & Attribute API test", () => {
    ...
    test("Get all category", async () => {
        const [response, responseBody] = await apiUtils.get(endPoints.categoryGetAll);
        expect(response.ok()).toBeTruthy();
	    expect(responseBody).toBeTruthy();
    });
...
// ToDo: create category with parent category
    ...
    test("Create attribute", async() => {
        const [response, responseBody] = await apiUtils.post(endPoints.attributeCreate(category_id), { data: payloads.attributeCreate(), headers: adminAuth })
        expect(response.ok()).toBeTruthy();
	    expect(responseBody).toBeTruthy();
        const res = await response.json();
    ...
    test("Update attribute", async () => {
        const [response, responseBody] = await apiUtils.put(endPoints.attributeUpdate(category_id, attribute_id), { data: payloads.attributeUpdate(), headers: adminAuth })
        expect(response.ok()).toBeTruthy();
	    expect(responseBody).toBeTruthy();
    })
