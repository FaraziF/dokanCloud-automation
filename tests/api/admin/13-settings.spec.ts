...
test.describe("general setting", () => {
    ...
    test.describe("Basic settings", () => {
        test("get general settings basic", async() => {
    ...
    test.describe("Domain", () => {
        ...
        test("add custom domain", async() => {
            ...
            const [response, responseBody] = await apiUtils.post(endPoints.getDomainSettings, {data: domainName});
            expect(response.ok()).toBeTruthy();
            expect(responseBody).toBeTruthy();
        })
        test("add refresh custom domain", async() => {
            const [response, responseBody] = await apiUtils.post(endPoints.getRefreshCustomDomain);
            expect(response.ok()).toBeTruthy();
            expect(responseBody).toBeTruthy();
        })
        test("edit custom domain", async() => {
            const domainNameUpdate= {domain: "farazi.com"}
            const [response, responseBody] = await apiUtils.patch(endPoints.editCustomDomain, {data:domainNameUpdate});
            expect(response.ok()).toBeTruthy();
            expect(responseBody).toBeTruthy();
        })
        test("remove custom domain", async() => {
            const [response, responseBody] = await apiUtils.delete(endPoints.removeCustomDomain);
            expect(response.ok()).toBeTruthy();
            expect(responseBody).toBeTruthy();
        })    
    })
...
test.describe("team settings", () => {
    test("get all team member", async() => {
        const [response, responseBody] = await apiUtils.get(endPoints.getAllTeamMember);
		expect(response.ok()).toBeTruthy();
		expect(responseBody).toBeTruthy();
    })
    ...
    */
    ...
    test("resend invitation invited team member", async() => {
        ...
        const [response, responseBody] = await apiUtils.post(endPoints.resendInvitationInvitedTeamMember, {data: invitedTeamMemberEmail2} );
...
test.describe("Billing", () => {
    ...
    test("get choose plan", async() => {
        const [response, responseBody] = await apiUtils.get(endPoints.getChoosePlans);
		expect(response.ok()).toBeTruthy();
		expect(responseBody).toBeTruthy();
    })
})
...
test.describe("Payment settings", () => {
   ...
   /*  test("update Stripe test credential", async() => {
    ...
    test("enable test gateway", async() => {
        ...
        const [response, responseBody] = await apiUtils.put(endPoints.enableTestGateway, {data: testGateway});
		expect(response.ok()).toBeTruthy();
		expect(responseBody).toBeTruthy();
    }) 
})
...
test.describe("SEO settings", () => {
    ...
    test("save social share", async() => {
        const [response, responseBody] = await apiUtils.put(endPoints.saveSocialShare, {data: payloads.saveAdvanceSEO()});
		expect(response.ok()).toBeTruthy();
		expect(responseBody).toBeTruthy();
    }) 
...
test.describe("Policies Settings", () => {
    test("get all policies", async() => {
        const [response, responseBody] = await apiUtils.get(endPoints.getPolicies);
		expect(response.ok()).toBeTruthy();
		expect(responseBody).toBeTruthy();
    }) 
})
...
test.describe("Admin profile", () => {
    test("get admin profile", async() => {
        const [response, responseBody] = await apiUtils.get(endPoints.getAdminProfile);
		expect(response.ok()).toBeTruthy();
		expect(responseBody).toBeTruthy();
    }) 
})
